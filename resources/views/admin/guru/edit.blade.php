            ...
            <div class="rounded-box bg-base-100 shadow-md">
                ...
                <form action="{{ route('admin.guru.update', $guru->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="form-control">
                        ...
                        <label
                            ...
                            <div id="upload-placeholder"
                                class="{{ $guru->foto ? 'hidden' : '' }} flex flex-col items-center gap-2">
                                ...
                                <p class="text-sm text-base-content/70">Klik untuk ganti foto</p>
                            </div>

                            <img id="preview-foto" src="{{ $guru->foto ? asset('storage/' . $guru->foto) : '' }}"
                                class="{{ $guru->foto ? '' : 'hidden' }} h-12 w-12 rounded-lg object-cover border"
                                alt="Preview Foto">
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" name="nip" class="input input-bordered w-full" value="{{ $guru->nip }}"
                            required>
                    </div>
                    ...
                    <div class="form-control">
                        ...
                        <select id="jabatan" name="jabatan" class="select select-bordered w-full">
                            ...
                            <option value="wali kelas" @selected(old('jabatan', strtolower($guru->jabatan)) === 'wali kelas')>
                                Wali Kelas
                            </option>
                            <option value="guru mapel" @selected(old('jabatan', strtolower($guru->jabatan)) === 'guru mapel')>
                                Guru Mapel
                            </option>
                            <option value="guru kelas" @selected(old('jabatan', strtolower($guru->jabatan)) === 'guru kelas')>
                                Guru Kelas
                            </option>
                        </select>
                    </div>
                    ...
                    <div id="wali-kelas-wrapper" style="display:none;" class="form-control">
                        ...
                        <select name="wali_kelas_id" class="select select-bordered w-full">
                            @foreach ($kelas as $k)
                                <option value="{{ $k->id }}" @selected(old('wali_kelas_id', $guru->wali_kelas_id) == $k->id)>
                                    {{ $k->nama_kelas }}
                                </option>
                            @endforeach
                        </select>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        function toggle() {
                            ...
                            console.log('ISI JABATAN:', value);

                            if (value === 'wali kelas') {
                                wali.style.display = 'block';
                            } else {
                                wali.style.display = 'none';
                                wali.querySelector('select').value = '';
                            }
                        }

                        jabatan.addEventListener('input', toggle);
                        jabatan.addEventListener('change', toggle);
